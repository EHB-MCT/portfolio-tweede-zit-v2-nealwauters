import React, { useEffect, useState } from 'react';
import { useLocation } from 'react-router-dom';

const QuestionDetails = () => {
  const location = useLocation();
  const data = location?.state?.data;
  console.log('data: ', data);
  const [answerData, setAnswerData] = useState(null);
  const [showModel, setShowModel] = useState(false);
  console.log('answerData: ', answerData);
  const [showteacherModel, setShowteacherModel] = useState(false);
  const [userProfile, setUserProfile] = useState(null);
  const [commentData, setCommentData] = useState(null);
  console.log('commentData: ', commentData);
  const [comment, setComment] = useState(null);
  const [render, setRender] = useState(false);
  console.log('userProfile: ', userProfile);
  const userId = localStorage.getItem('id');

  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(
          `http://localhost:5000/api/answers/${data?._id}`,
          {
            method: 'GET',
            headers: {
              'Content-Type': 'application/json',
            },
          }
        );

        if (!response.ok) {
          throw new Error(`Error: ${response.status}`);
        }

        const result = await response.json();
        setAnswerData(result);
      } catch (error) {
        console.error('Error fetching data:', error);
        throw error;
      }
    };
    fetchData();
  }, [render]);

  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(
          `http://localhost:5000/api/users/user/${userId}`,
          {
            method: 'GET',
            headers: {
              'Content-Type': 'application/json',
            },
          }
        );

        if (!response.ok) {
          throw new Error(`Error: ${response.status}`);
        }

        const result = await response.json();
        setUserProfile(result);
      } catch (error) {
        console.error('Error fetching data:', error);
        throw error;
      }
    };
    fetchData();
  }, []);
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(
          `http://localhost:5000/api/comments/${data?._id}`,
          {
            method: 'GET',
            headers: {
              'Content-Type': 'application/json',
            },
          }
        );

        if (!response.ok) {
          throw new Error(`Error: ${response.status}`);
        }

        const result = await response.json();
        setCommentData(result);
      } catch (error) {
        console.error('Error fetching data:', error);
        throw error;
      }
    };
    fetchData();
  }, [render]);

  const handleCreateQuestion = () => {
    if (userId !== null || userId !== undefined) {
      if (userProfile?.role == 'teacher') {
        setShowModel(!showModel);
      } else {
        setShowteacherModel(!showteacherModel);
      }
    } else {
      navigate('/user');
    }
  };

  const handleComment = async (e) => {
    e.preventDefault();
    const bodydata = {
      question: data?._id,
      content: comment,
      author: userId,
    };
    try {
      const response = await fetch('http://localhost:5000/api/comments', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify(bodydata),
      });

      if (!response.ok) {
        throw new Error(`Error: ${response.status}`);
      }

      const result = await response.json();
      console.log('result: ', result);
      setRender(!render);
    } catch (error) {
      console.error('Error posting data:', error);
      throw error;
    }
  };
  return (
    <div class='bg-white sm:px-6 px-4 py-10 font-[sans-serif]'>
      <div class='max-w-xl mx-auto'>
        <div>
          <h2 class='text-3xl font-bold text-[#333] inline-block'>
            {data?.title}
          </h2>
          <p class='text-gray-400 text-sm mt-4'>{data?.content}</p>
        </div>
        <div class='flex items-center gap-3 pt-3'>
          <p class='text-xs text-gray-400'>BY: {data?.auther?.username}</p>
          <span class='text-sm block text-gray-400'>
            {data?.createdAt?.split('T')[0]}
          </span>
        </div>
        <button
          onClick={handleCreateQuestion}
          className='flex items-center justify-end bg-blue-700 text-white transition-all ease px-3 py-1 rounded-full ml-auto mr-0'
        >
          Write Answer
        </button>
        <hr class='my-5' />
        <div class='grid gap-4'>
          <div className='w-full'>
            <div class='cursor-pointer rounded overflow-hidden border-b-2 pb-4'>
              <div>
                <h3 class='text-xl font-bold text-[#333] transition-all'>
                  Answers
                </h3>
                {data?.correctAnswer && (
                  <div class='mt-6 group'>
                    <button className='flex items-center justify-end bg-green-700 text-white text-xs transition-all ease px-3 py-[2px] my-1 rounded-full'>
                      Correct Answer
                    </button>
                    <p class='text-xs text-gray-500'>
                      BY: {data?.author?.username}
                    </p>
                    <p class='text-gray-400 text-sm'>{data?.content}</p>
                  </div>
                )}
                {answerData &&
                  answerData?.map((mapdata) => (
                    <div class='mt-6 group'>
                      <p class='text-xs text-gray-500'>
                        BY: {mapdata?.author?.username}
                      </p>
                      <p class='text-gray-400 text-sm'>{mapdata?.content}</p>
                      {userId == data?._id && (
                        <button className='hidden group-hover:flex items-center justify-end bg-blue-700 text-white transition-all ease px-3 py-1 rounded-full ml-auto mr-0'>
                          Correct
                        </button>
                      )}
                    </div>
                  ))}
              </div>
            </div>
            <div className='w-full group'>
              <p class='text-lg text-gray-500 py-2'>Comments</p>
              <form onSubmit={handleComment} className='w-full'>
                <textarea
                  onChange={(e) => setComment(e.target.value)}
                  value={comment}
                  rows={2}
                  required
                  className='w-full border outline-0 px-3 py-1 rounded-xl'
                  placeholder='write comment...'
                />
                <button
                  type='submit'
                  className='flex items-center justify-end bg-blue-700 text-white px-3 py-1 rounded-full ml-auto mr-0'
                >
                  submit
                </button>
              </form>
              {commentData?.map((items) => (
                <div className='ml-4 mb-3'>
                  <p class='text-xs text-gray-400'>
                    BY: {items?.author?.username}
                  </p>
                  <p className='text-gray-400 ml-2'>{items?.content}</p>
                </div>
              ))}
            </div>
          </div>
        </div>
      </div>
      {showModel && (
        <Model
          handleShowModel={handleCreateQuestion}
          userProfile={userProfile}
          render={render}
          setRender={setRender}
          questionId={data?._id}
        />
      )}
      {showteacherModel && (
        <TeacherModel handleShowModel={handleCreateQuestion} />
      )}
    </div>
  );
};

const Model = ({ handleShowModel, setRender, render, questionId }) => {
  const [content, setContent] = useState(null);
  const author = localStorage.getItem('id');
  const handlecreateQuestion = async (e) => {
    e.preventDefault();
    const data = {
      content: content,
      author: author,
      question: questionId,
    };
    try {
      const response = await fetch(`http://localhost:5000/api/answers/`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify(data),
      });

      if (!response.ok) {
        throw new Error(`Error: ${response.status}`);
      }

      const result = await response.json();
      console.log('result: ', result);
      setRender(!render);
      handleShowModel();
    } catch (error) {
      console.error('Error posting data:', error);
      throw error;
    }
  };
  return (
    <div class='fixed inset-0 p-4 flex flex-wrap justify-center items-center w-full h-full z-[1000] before:fixed before:inset-0 before:w-full before:h-full before:bg-[rgba(0,0,0,0.5)] overflow-auto font-[sans-serif]'>
      <form
        onSubmit={handlecreateQuestion}
        class='w-full max-w-lg bg-white shadow-lg rounded-md p-8 relative'
      >
        <svg
          onClick={handleShowModel}
          xmlns='http://www.w3.org/2000/svg'
          class='w-3.5 cursor-pointer shrink-0 fill-gray-800 hover:fill-red-500 float-right'
          viewBox='0 0 320.591 320.591'
        >
          <path
            d='M30.391 318.583a30.37 30.37 0 0 1-21.56-7.288c-11.774-11.844-11.774-30.973 0-42.817L266.643 10.665c12.246-11.459 31.462-10.822 42.921 1.424 10.362 11.074 10.966 28.095 1.414 39.875L51.647 311.295a30.366 30.366 0 0 1-21.256 7.288z'
            data-original='#000000'
          ></path>
          <path
            d='M287.9 318.583a30.37 30.37 0 0 1-21.257-8.806L8.83 51.963C-2.078 39.225-.595 20.055 12.143 9.146c11.369-9.736 28.136-9.736 39.504 0l259.331 257.813c12.243 11.462 12.876 30.679 1.414 42.922-.456.487-.927.958-1.414 1.414a30.368 30.368 0 0 1-23.078 7.288z'
            data-original='#000000'
          ></path>
        </svg>
        <div class='my-8'>
          <h4 class='text-2xl text-center text-gray-800 font-bold'>
            Answer the Question
          </h4>
          <label class='text-sm text-gray-700 '>Answer</label>
          <textarea
            onChange={(e) => setContent(e.target.value)}
            value={content}
            type='text'
            rows={4}
            placeholder='Enter detailed Answer'
            class='px-4 py-2.5 bg-[#f0f1f2] text-gray-800 w-full text-sm focus:bg-transparent outline-blue-600 rounded-md'
          />
        </div>
        <button
          type='submit'
          class='px-5 py-2.5 w-full rounded-md text-white text-sm outline-none bg-blue-600 hover:bg-blue-700'
        >
          Create
        </button>
      </form>
    </div>
  );
};

const TeacherModel = ({ handleShowModel }) => {
  return (
    <div class='fixed inset-0 p-4 flex flex-wrap justify-center items-center w-full h-full z-[1000] before:fixed before:inset-0 before:w-full before:h-full before:bg-[rgba(0,0,0,0.5)] overflow-auto font-[sans-serif]'>
      <form class='w-full max-w-lg bg-white shadow-lg rounded-md p-8 relative'>
        <svg
          onClick={handleShowModel}
          xmlns='http://www.w3.org/2000/svg'
          class='w-3.5 cursor-pointer shrink-0 fill-gray-800 hover:fill-red-500 float-right'
          viewBox='0 0 320.591 320.591'
        >
          <path
            d='M30.391 318.583a30.37 30.37 0 0 1-21.56-7.288c-11.774-11.844-11.774-30.973 0-42.817L266.643 10.665c12.246-11.459 31.462-10.822 42.921 1.424 10.362 11.074 10.966 28.095 1.414 39.875L51.647 311.295a30.366 30.366 0 0 1-21.256 7.288z'
            data-original='#000000'
          ></path>
          <path
            d='M287.9 318.583a30.37 30.37 0 0 1-21.257-8.806L8.83 51.963C-2.078 39.225-.595 20.055 12.143 9.146c11.369-9.736 28.136-9.736 39.504 0l259.331 257.813c12.243 11.462 12.876 30.679 1.414 42.922-.456.487-.927.958-1.414 1.414a30.368 30.368 0 0 1-23.078 7.288z'
            data-original='#000000'
          ></path>
        </svg>
        <div class='my-8'>
          <h4 class='text-2xl text-center text-gray-800 font-bold'>
            Just Teacher can Answer the Question
          </h4>
        </div>
        <button
          type='button'
          onClick={handleShowModel}
          class='px-5 py-2.5 w-full rounded-md text-white text-sm outline-none bg-blue-600 hover:bg-blue-700'
        >
          Okay
        </button>
      </form>
    </div>
  );
};

export default QuestionDetails;
